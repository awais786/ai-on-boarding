...
|---|---|---|---|
| 1 | Lint the files the PR changed. Mechanical, always right about what it covers. | - | `pr_review/pipeline/lint.py` |
| 2 | Judge the diff for architectural fit, convention compliance, correctness/completeness, and blast radius (duplicated logic elsewhere in the repo). Runs Layer 1 itself first; merges any of its own findings landing near a Layer 1 finding into it rather than dropping either. | `claude-sonnet-5` | `pr_review/pipeline/judge.py` |
| 3 | Independently re-check every Layer 2 finding's citation and failure scenario before it can block a merge, one finding per agent call. Drops what doesn't hold up; can only escalate a severity, never lower one. | `claude-haiku-4-5` (`claude-sonnet-5` for CRITICAL) | `pr_review/pipeline/verify.py` |
| 4 | Render the verdict, per this repo's review contract: a finding blocks only if it still carries a citation **and** its severity is CRITICAL or MAJOR. Exits non-zero on `no`, for CI. | - | `pr_review/pipeline/gate.py` |

## Layout

```
claude-pr-review/
├── pr_review/              the installable package (pip install -e .)
│   ├── pipeline/           the four layers above: lint, judge, verify, gate
│   ├── core/               shared agent loop (agent.py), read-only tools, target resolution
│   ├── lib/                pure logic, no I/O: severity map, citations/fingerprint, sticky-comment state
│   └── ci/                 everything that talks to GitHub: github.py, post.py, and run.py (the CI entrypoint)
├── prompts/                judge.md, verify.md - the two system prompts
├── evals/                  scoring harness + frozen fixtures (see "Testing without a live PR")
├── tests/                  one test file per module, run directly - no pytest
├── pyproject.toml          makes `pr_review` importable from anywhere via an editable install
└── Makefile, requirements.txt, ruff.toml
```

`pr_review` is a real installed package (not a `sys.path` hack), so every file under `pr_review/`,
`evals/`, and `tests/` imports it the same way - `from pr_review.pipeline import judge` - and runs
correctly whether invoked directly (`python3 tests/test_x.py`) or via `-m`.
...
.venv/bin/pip install -e .     # makes `pr_review` importable everywhere below

# Needs an Anthropic credential - ANTHROPIC_API_KEY, or `ant auth login`.
export ANTHROPIC_API_KEY=...

# The whole pipeline, for one PR push - what CI actually runs:
.venv/bin/python -m pr_review.ci.run --repo owner/repo --pr 123 --head-sha <sha>

# Or one layer at a time:
.venv/bin/python -m pr_review.pipeline.lint [pr-number|branch]        # omit for the working tree
.venv/bin/python -m pr_review.pipeline.judge [pr-number|branch] --out build/findings.json
.venv/bin/python -m pr_review.pipeline.verify [pr-number|branch] --findings build/findings.json --out build/verified_findings.json
.venv/bin/python -m pr_review.pipeline.gate --findings build/verified_findings.json --out build/verdict.md
```

All three accept the same target: a PR number, a branch name, or nothing (the working tree's
uncommitted and staged changes). `pr_review.core.target` resolves it the same way for every layer,
so they never silently disagree about which files or diff make up "the PR".

Tests run directly, no pytest: `.venv/bin/python3 tests/test_gate_severity.py`.
...
Offline, every push: 261 tests, including `test_lifecycle.py`, which drives several pushes on one
...
Path filtering (e.g. skip non-code PRs), and a larger fixture set - `evals/fixtures/` has eight
