...
verify.precheck()'s `source == "lint"` branch before the `ruff:` branch
...
from pr_review.pipeline import gate, judge, lint, verify
...
def test_lint_findings_are_confirmed_not_rejected():
    ...
    for f in findings:
        assert verify.precheck(f, _Ctx()) is verify.Precheck.CONFIRMED
...
def _selected_prefixes() -> list[str]:
    text = (Path(__file__).resolve().parents[1] / "ruff.toml").read_text()  # tests/ -> claude-pr-review/ruff.toml
