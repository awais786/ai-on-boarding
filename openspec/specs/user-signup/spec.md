...
## Purpose
Lets a person create an account by providing an email address and a password, so they can
subsequently sign in. The email address is the account's only identifier - there is no
separate username.
...
- **THEN** the response is HTTP 200 with a body containing only the email field
