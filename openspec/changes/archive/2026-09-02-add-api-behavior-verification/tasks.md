...
- [x] 1.1 Create `tooling/api-verification/` with its own Python requirements file (the Anthropic
...
- [x] 1.2 Give the tooling its own pytest configuration so it is a self-contained suite separate
...
- [x] 2.1 Write the route enumerator that walks Django's URL resolver and emits every routed
...
- [x] 2.2 Generate the OpenAPI description into a build directory, in a mode that does not treat
  the known health-endpoint generation error as fatal, and verify the five API operations appear
  in the generated file
- [x] 2.3 Write `surfaces.yaml` declaring the reset page as a spec-bearing surface the description
...
- [x] 2.4 Write the completeness check - routed addresses minus described minus declared minus
...
- [x] 3.1 Define the library format - one file per capability, each entry naming its requirement
...
- [x] 3.2 Write the checks for `user-signup` from `openspec/specs/user-signup/spec.md`, working
  from the spec text, and verify every requirement in that spec is either covered by an entry or
  recorded in the register
- [x] 3.3 Write the checks for `user-signin` the same way, including the sequence that compares
...
- [x] 3.4 Write the checks for `user-password-reset` the same way, including the sequences that
...
- [x] 3.5 Write `unobservable.yaml` recording each requirement or scenario that cannot be judged
...
- [x] 3.6 Write the collection builder that converts the description, attaches the library's
...
- [x] 4.1 Write the run script that starts the mail catcher, starts the server with the existing
  ...
  recording an outcome for every check, with each recorded failure traceable to a response the
  promoted specs contradict rather than to the tooling
- [x] 4.2 Give every sequence run-unique account addresses so the reset rate limit and the signin
...
- [x] 4.3 Verify the reset sequences take the code from the delivered message rather than from any
  response, and that the check asserting the reset-request response does not carry the code is
  present and holds when exercised against a delivered code

## 5. Evaluation and the gate

- [x] 5.1 Write the evaluation step that reads the promoted specs, the built collection, the check
...
- [x] 5.2 Make the evaluation fail with a message naming what is missing when its credential is
...
- [x] 5.3 Write the gate that fails on a completeness failure, on any failed check in the report,
...
- [x] 6.1 Add the GitHub Actions workflow triggered on push to `main` and on manual dispatch,
...
- [x] 6.2 Have the run publish the generated description, the built collection, the run report,
...
- [x] 7.1 Write `tooling/api-verification/README.md` stating which part supplies the endpoint set,
...
- [x] 7.2 Document what a contributor does when a run fails on a coverage gap - add a check, or
  add a register entry with a category and reason - and verify the instruction names both options
  and where each is written
- [x] 7.3 Link the tooling README from `README.md` and `sdd_django_demo/README.md`, and verify
  both links resolve to the file

## 8. Tests (after implementation, from the spec)

- [x] 8.1 List every requirement in this change's `specs/api-behavior-verification/spec.md` and
  what a test would need to assert for each, working only from the spec
- [x] 8.2 Write the tests into `tooling/api-verification/tests/` from that list, covering each
  requirement, naming each test after the behaviour it protects
- [x] 8.3 Assert directly, not through a success path, that the evaluation fails rather than
  passes when its credential is absent, and that a run whose report holds a failed check cannot
  exit zero
- [x] 8.4 Run the tooling suite and the Django project's suite and confirm every test in both
  passes
- [x] 8.5 Prove at least one new test can actually fail - remove a check entry whose requirement
  is not in the register, confirm the gate goes red and names that requirement, then restore it

## 9. Traceability and review

- [x] 9.1 Build `traceability.md` mapping every requirement in this change to the file that
  implements it and the test that covers it
- [x] 9.2 Run `/code-review` and record the verdict
- [x] 9.3 Fix any blocking findings, each cited to a requirement, a named failing test, or a
  documented convention
- [x] 9.4 Run `/code-review` again (round 2, verify-only) and confirm `Ready to merge: yes`
