...
| Limit how often a reset may be requested for one address | `api/views.py:PasswordResetAddressThrottle`, `settings.py:REST_FRAMEWORK['DEFAULT_THROTTLE_RATES']` | `test_requests_beyond_the_limit_issue_and_send_nothing`, `test_a_flood_cannot_leave_an_address_with_no_usable_code`, `test_being_limited_does_not_reveal_whether_an_account_exists`, `test_the_limit_is_per_address_not_global` |
