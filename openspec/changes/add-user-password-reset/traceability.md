...
| Supersede an earlier unused code | `api/models.py:PasswordResetCode.issue_for` (bulk `update(usable=False)` before issuing, one retry on `IntegrityError`), `Meta.constraints` (partial unique index) | `test_requesting_again_kills_the_earlier_code`, `test_an_account_can_never_hold_two_usable_codes`, `test_issuing_retries_when_the_digest_collides`, `test_issuing_gives_up_when_the_collision_persists` |
