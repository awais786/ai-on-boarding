"""The only place this server talks to the backend API.
...
secret_pages on_submit closure does that. Username masking on reads lives in
the read tools themselves (tools/read.py), not here - this module's job
is talking to the backend, not deciding what a caller is allowed to see.
"""
...
_client = httpx2.AsyncClient(base_url=BACKEND_API_BASE, timeout=REQUEST_TIMEOUT)
...
def validate_password_strength(password):
    ...
    if len(password) < 8 or not re.search(r'[A-Za-z]', password) or not re.search(r'\d', password):
        raise BackendAPIError('Must be at least 8 characters and contain a letter and a digit.')
...
async def exchange_google_token(google_access_token):
    ...
    if response.status_code == 403:
        raise BackendAPIError(extract_error_detail(response), no_account=True)
    if response.status_code != 200:
        raise BackendAPIError(extract_error_detail(response))
...
class AuthedBackendClient:
    """Calls that need the caller's own backend token, held as instance state
