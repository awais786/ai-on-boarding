...
against django_client's mirror of Django's password rule.
...
def test_password_strength_cases(case):
    if case['valid']:
        django_client.validate_password_strength(case['password'])  # must not raise
    else:
        with pytest.raises(django_client.DjangoAPIError):
            django_client.validate_password_strength(case['password'])
