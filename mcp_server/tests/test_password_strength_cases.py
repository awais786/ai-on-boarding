...
against validation.py's mirror of Django's password rule.
...
def test_password_strength_cases(case):
    if case['valid']:
        validation.validate_password_strength(case['password'])  # must not raise
    else:
        with pytest.raises(ValidationError):
            validation.validate_password_strength(case['password'])
