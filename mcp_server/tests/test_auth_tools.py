...
elicitation.py): it takes no password as a tool argument, instead pointing the
...
async def test_signup_rejects_a_duplicate_email(sign_in, django, drive_secret_tool):
    django.exchange_error = BackendAPIError('No account.', no_account=True)
    await sign_in(GOOGLE_A)
    django.signup_error = BackendAPIError(
...
async def test_reset_password_rejects_an_invalid_code(django, drive_secret_tool):
    django.reset_error = BackendAPIError('That reset link is not valid.')
...
async def test_change_my_password_rejects_a_wrong_current_password(sign_in, django, drive_secret_tool):
    await sign_in(GOOGLE_A)
    django.change_own_password_error = BackendAPIError(
