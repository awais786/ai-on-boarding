"""Tests for tools/read.py - the read-only admin listing tools.

Username masking lives here, not in backend_client (see backend_client.py's
...
directly against this tool layer rather than through backend_client.
"""
...
async def test_list_users_by_country_masks_too(sign_in, django):
    ...
    result = await tools.read.list_users_by_country('GB')
