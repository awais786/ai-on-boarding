...
@pytest.mark.django_db
def test_signin_failures_spread_across_window_do_not_lock_out(client, user):
    """Three failures each < 5 minutes apart, but not all within one 5-minute window."""
    start = timezone.now()
    for minutes in (0, 4, 8):
        with patch('api.views.timezone.now', return_value=start + timedelta(minutes=minutes)):
            signin(client, 'ada@example.com', password='wrongpassword')

    with patch('api.views.timezone.now', return_value=start + timedelta(minutes=8)):
        response = signin(client, 'ada@example.com', password='lovelace1')
...
@pytest.mark.django_db
def test_signin_three_failures_within_one_window_lock_out(client, user):
    start = timezone.now()
    for minutes in (0, 2, 4):
        with patch('api.views.timezone.now', return_value=start + timedelta(minutes=minutes)):
            signin(client, 'ada@example.com', password='wrongpassword')

    with patch('api.views.timezone.now', return_value=start + timedelta(minutes=4, seconds=30)):
        response = signin(client, 'ada@example.com', password='lovelace1')
...
def test_signin_embargo_rejection_matches_wrong_password_rejection(client):
    create_account(username='ada', email='ada@example.com', password='lovelace1', country='India')
    create_account(username='grace', email='grace@example.com', password='hopper1')

    embargoed = signin(client, 'ada@example.com', password='lovelace1')
    wrong_password = signin(client, 'grace@example.com', password='wrongpass1')
