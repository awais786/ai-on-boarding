...
def build_propose_prompt(report_text: str) -> str:
    return (
        ...
        "Granularity rules:\n"
        "- One issue per core code change (one middleware, one config change, "
        "one listener, etc.) - not one issue per test, doc, or review task.\n"
        "- Fold that change's own tests into its own issue's Tests section. "
        "Tests are written as part of the same unit of work as the code they "
        "cover, not split into a separate issue.\n"
        "- Do NOT create separate issues for documentation, performance "
        "testing, or security review unless the report identifies one as "
        "genuinely blocking, load-bearing work distinct from implementing "
        "the core change - default to omitting them rather than including "
        "them speculatively.\n\n"
