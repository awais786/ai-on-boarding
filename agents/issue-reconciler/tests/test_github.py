        ...
        [
            {
                "node": {
                    "items": {
                        ...
                        "nodes": [
                            {"id": "item-1", "status": {"name": "Todo"}, "content": {"__typename": "Issue", "number": 1, "repository": {"nameWithOwner": "awais786/ai-on-boarding"}}},
                            {"id": "item-2", "status": {"name": "Done"}, "content": {"__typename": "Issue", "number": 2, "repository": {"nameWithOwner": "someone-else/some-other-repo"}}},
                            {"id": "item-3", "status": None, "content": {"__typename": "PullRequest"}},
                            {"id": "item-5", "status": None, "content": {"__typename": "Issue", "number": 5, "repository": {"nameWithOwner": "a-fork/ai-on-boarding"}}},
                        ],
                    }
                }
            },
            {"node": {"items": {"pageInfo": {"hasNextPage": False, "endCursor": None}, "nodes": [{"id": "item-4", "status": None, "content": {"__typename": "Issue", "number": 4, "repository": {"nameWithOwner": "awais786/ai-on-boarding"}}}]}}},
        ]
    )

    items = fetch_board_items(client)

    # item-5 is excluded: same repo *name* under a different owner (a fork) -
    # this is exactly what the owner+name check (not name-only) must reject.
